  ...
  if (isLoading) {
    return (
      <div className="min-h-screen relative">
        <Navbar />
        {/* BG dotted */}
        <div className="absolute inset-0 -z-10 h-full w-full bg-white bg-[radial-gradient(#d7dae0_1.5px,transparent_1px)] [background-size:16px_16px]">
        ...
        {filteredChapters.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
            {filteredChapters.map((chapter: TChapterData, index: number) => (
              ...
              >
                {/* Decorative Corner */}
                <div className="absolute top-0 right-0 w-20 h-20 bg-gradient-to-bl from-teal-50 to-transparent rounded-bl-full opacity-0 group-hover:opacity-100 transition-opacity"></div>

                {/* Top Section */}
                <div className="flex justify-between items-start mb-4">
                  <div className="flex items-center justify-center w-12 h-12 rounded-full bg-gradient-to-br from-teal-500 to-teal-600 text-white font-bold text-lg shadow-md">
                    {index + 1}
                  </div>
                  <div className="text-right">
                ...
                {/* Bottom Accent Line */}
                <div className="absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r from-teal-500 to-teal-400 transform scale-x-0 group-hover:scale-x-100 transition-transform origin-left"></div>
              </div>
            ))}
          </div>
        ) : (
